...
数据转化到dom节点  做渲染  先要转换树结构再用一个方法转成一个节点 放到节点里面去
...
对应id=2的子数据是什么 即pid=2的数据 放到id=2的children数组里
...
console.log(treeData); 通过用于另外一个函数转成vue里面的动态路由   后端接收数据  树形结构化  再去形成路由
